...
A GraphQL server for [curated Open Source packages](https://github.com/BVPMOSC/mosc-curated-list), powered by Apollo.

Set the `GITHUB_TOKEN` env var with a GitHub OAuth token.

Set the `SOURCE_REPO` env var with the repo containing the `PACKAGES.md` file on the root of the `master` branch. For example: `SOURCE_REPO=BVPMOSC/mosc-curated-list`.
